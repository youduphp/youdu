...
class User
{
    ...
    {
        $resp = $this->client->get($this->app->url('/cgi/user/simplelist'), ['deptId' => $deptId]);
    ...
    {
        $resp = $this->client->get($this->app->url('/cgi/user/list'), ['deptId' => $deptId]);
        $decoded = json_decode($resp['body'], true, 512, JSON_THROW_ON_ERROR);
    ...
    public function create($userId, string $name, int $gender = 0, string $mobile = '', string $phone = '', string $email = '', array $dept = []): bool
    {
        ...
        $resp = $this->client->post($this->app->url('/cgi/user/create'), $parameters);
    ...
    public function update($userId, string $name, int $gender = 0, string $mobile = '', string $phone = '', string $email = '', array $dept = []): bool
    {
        ...
        $resp = $this->client->post($this->app->url('/cgi/user/update'), $parameters);
    ...
    public function updatePosition($userId, int $deptId, string $position = '', int $weight = 0, int $sortId = 0): bool
    {
        ...
        $resp = $this->client->post($this->app->url('/cgi/user/positionupdate'), $parameters);
    ...
    public function delete($userId): bool
    {
        if (is_array($userId)) {
            ...
            $resp = $this->client->post($this->app->url('/cgi/user/batchdelete'), $parameters);
        ...
        // single delete
        $resp = $this->client->get($this->app->url('/cgi/user/delete'), ['userId' => $userId]);
        $decoded = json_decode($resp['body'], true, 512, JSON_THROW_ON_ERROR);
    ...
    {
        $resp = $this->client->get($this->app->url('/cgi/user/get'), ['userId' => $userId]);
        $decoded = json_decode($resp['body'], true, 512, JSON_THROW_ON_ERROR);
    ...
    public function setAuth($userId, int $authType = 0, string $passwd = ''): bool
    {
        ...
        $resp = $this->client->post($this->app->url('/cgi/user/setauth'), $parameters);
    ...
    {
        $resp = $this->client->get($this->app->url('/cgi/avatar/get'), ['userId' => $userId, 'size' => $size]);
    ...
    {
        $resp = $this->client->get($this->app->url('/cgi/identify?token=' . $token, false));
